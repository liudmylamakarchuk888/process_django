...
def get_mapdict():
    map_dict = {}
    csv_path = os.path.join(settings.CSV_FOLDER, 'mapping.csv')
...
def get_video_summary(items_list):
    ...
    sorted_cats = sorted(
        cat_dict.items(), key=lambda x: x[1], reverse=True)[:5]
    sorted_objs = sorted(
        obj_dict.items(), key=lambda x: x[1], reverse=True)[:5]
    sorted_places = sorted(
        place_dict.items(), key=lambda x: x[1], reverse=True)[:5]
    sorted_acts = sorted(
        act_dict.items(), key=lambda x: x[1], reverse=True)[:5]
    sorted_subjs = sorted(
        subj_dict.items(), key=lambda x: x[1], reverse=True)[:5]
